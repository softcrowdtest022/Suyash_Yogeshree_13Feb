...
const AddDimensions = ({ open, onClose, onAdd }) => {
  ...
  const calculateWeight = () => {
    ...
    if (Thickness && Width && Length && Density) {
      const thicknessMm = parseFloat(Thickness) / 1000; // Convert mm to meters
      const widthMm = parseFloat(Width) / 1000;
      const lengthMm = parseFloat(Length) / 1000;
      const density = parseFloat(Density);
      
      const volume = thicknessMm * widthMm * lengthMm; // Volume in cubic meters
      const weight = volume * density * 1000; // Convert to kg (density is g/cm³)
                ...
                <Grid container spacing={1}>
                  ...
                  <Grid item xs={6}>
                    <Typography variant="body2" fontWeight={500} align="right">
                      {(formData.Thickness * formData.Width * formData.Length / 1000000000).toFixed(6)} m³
                    </Typography>
                  </Grid>
